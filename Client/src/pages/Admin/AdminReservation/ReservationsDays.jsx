...
export const ReservationsDays = () => {
  ...
  const onSubmit = async ()=>{
    try {
      if(days.length == 0){
        setMsg("Debes seleccionar algún día.");
      }else{
  ...
  return (
            ...
            <Row>
              <Col xs={12} className='d-flex justify-content-center'>
                <article className='form-days p-4 d-flex flex-column align-items-center'>
                  <p>Los días disponibles actualmente son: <span className='days fw-bold'> {availableDays?.map((elem)=> {return elem.reservation_day_name + ", "})}</span> quieres modificarlos?</p>
                  <div className='divisor mb-3'></div>
                  <p className='fw-bold fs-5'>Selecciona los nuevos días disponibles para reservar:</p>
                  <Form>
    ...
    <Button className='m-3 btn' onClick={onSubmit}>Modificar</Button>
    <span>{msg}</span>
                </article>
              </Col>
              {newDays &&
              ...
              <article className='msg-days p-2 mt-3'>
                <p> ✔ Se han modificado correctamente los días disponibles para reservar. </p>
              </article>
              </Col>}
            </Row>
          </Container>
